...
let users = {
    ...
    maxLang: function () {
        let maxLang = 0;
        this.getArray().forEach(item => {
            maxLang = item['langCount'] > maxLang ? item['langCount'] : maxLang;
        });
        return maxLang;
    },
    males: function () {
        let sexCount = 0;
        this.getArray().forEach(item => {
            sexCount = (item['sex'].toLowerCase() === 'male') ? ++sexCount : sexCount;
        });
        return sexCount;
    },
    females: function () {
        let sexCount = 0;
        this.getArray().forEach(item => {
            sexCount = (item['sex'].toLowerCase() === 'female') ? ++sexCount : sexCount;
        });
        return sexCount;
    }

};
document.getElementById('avgAge').innerText = users.avgAge();
document.getElementById('maxLang').innerText = users.maxLang();
document.getElementById('females').innerText = users.females();
document.getElementById('males').innerText = users.males();
